	...
	<view>
		...
		<view class="shop-content">
			...
			<view class="tabContent1" v-show="current==0">
				<view class="detail">
					<view class="detail-title">{{message.fccMessageTitle}}</view>
					<view class="detail-time">{{message.fccMessageTime | formatDate}}</view>
					<view class="detail-content" v-html="message.fccMessageContent"></view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getmessage(id){
				this.uni_request.get(this.request_list.getmessage+"?id="+id).then(res =>{
						if(res.code == 200) {
							this.message = res.data
							console.log(this.messages)
							uni.setNavigationBarTitle({
							                    title: res.data.fccMessageTitle
							                });
						} else{
							uni.showToast({
								icon: 'none',
...
<style scoped>
	...
	.detail{
		display: block;margin: 20rpx 30rpx 10rpx 30rpx;color: #000000;
	}
	.detail-title{
		font-size: 35rpx;
	}
	.detail-time {
		font-size: 20rpx;margin: 15rpx auto;
	}
	.detail-content{
		font-size: 28rpx;margin: 15rpx auto;
	}
